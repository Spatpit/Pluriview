...
const UBOL_ARCHIVE_PATH: &str =
    "assets/third_party/ubol/uBOLite_2026.714.1952.edge.zip";

fn expose_ubol_fingerprint() {
    println!("cargo:rerun-if-changed={UBOL_ARCHIVE_PATH}");
    let archive = std::fs::read(UBOL_ARCHIVE_PATH)
        .expect("read embedded uBlock Origin Lite archive");
    println!(
